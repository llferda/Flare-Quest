...
# Yeni fonksiyon: Başlangıç ekranını gösterir
def show_start_screen(window, cam):
    global vp_size_changed
    
    start_screen = True
    selected_option = 0  # 0: Başla, 1: Çıkış
    
    # Başlangıç ekranı paleti
    start_palette = palettes_dict["crystal"]
    
    while start_screen and not glfw.window_should_close(window):
        glfw.poll_events()
        ...
        if keyboard.is_pressed("UP") or keyboard.is_pressed("W"):
            selected_option = 0
            time.sleep(0.2)  # Tuş basımları arasında küçük bir gecikme
        ...
        if keyboard.is_pressed("ENTER") or keyboard.is_pressed("SPACE"):
            if selected_option == 0:  # Başla seçeneği
                start_screen = False
                time.sleep(0.2)
            elif selected_option == 1:  # Çıkış seçeneği
                glfw.destroy_window(window)
                quit()
        
        # Ekranı temizle
        glClearColor(start_palette["background"][0], start_palette["background"][1], start_palette["background"][2], 1)
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
        
        # Oyun başlığını çiz
        render_AN("CRYSTAL RETURN", start_palette["luna"], [-2.5, 3], cam, 0.2)
        
        # Menü seçeneklerini çiz
        başla_renk = start_palette["powerup_speed"] if selected_option == 0 else start_palette["luna"]
        çıkış_renk = start_palette["powerup_speed"] if selected_option == 1 else start_palette["luna"]
        
        render_AN("BASLA", başla_renk, [-0.9, 0], cam, 0.15)
        render_AN("CIKIS", çıkış_renk, [-0.9, -1.5], cam, 0.15)
        
        # Oyun açıklaması
        render_AN("OK TUSLARI ILE SECIM YAPIN", start_palette["obstacle"], [-3, -4], cam, 0.1)
...
# Yeni fonksiyon: Oyun sonu ekranını gösterir
def show_game_over_screen(window, cam, score, death_message):
    global vp_size_changed
    
    game_over_screen = True
    selected_option = 0  # 0: Tekrar Oyna, 1: Çıkış
    
    # Oyun sonu ekranı paleti
    end_palette = palettes_dict["crystal"]
    
    while game_over_screen and not glfw.window_should_close(window):
        glfw.poll_events()
        ...
        if keyboard.is_pressed("ENTER") or keyboard.is_pressed("SPACE"):
            if selected_option == 0:  # Tekrar Oyna seçeneği
                return True  # Ana oyun döngüsüne geri dön
            elif selected_option == 1:  # Çıkış seçeneği
                glfw.destroy_window(window)
                quit()
        
        # Ekranı temizle
        glClearColor(end_palette["background"][0], end_palette["background"][1], end_palette["background"][2], 1)
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
        
        # Game Over yazısını çiz
        render_AN("GAME OVER", end_palette["obstacle"], [-1.8, 3], cam, 0.2)
        
        # Skoru çiz
        render_AN("SCORE: " + str(round(score, 1)), end_palette["luna"], [-2, 1.5], cam, 0.15)
        
        # Ölüm mesajını çiz (eğer çok uzunsa kısalt)
        if len(death_message) > 30:
            death_message = death_message[:30] + "..."
        render_AN(death_message, end_palette["luna"], [-3, 0], cam, 0.1)
        
        # Menü seçeneklerini çiz
        tekrar_renk = end_palette["powerup_speed"] if selected_option == 0 else end_palette["luna"]
        çıkış_renk = end_palette["powerup_speed"] if selected_option == 1 else end_palette["luna"]
        
        render_AN("TEKRAR OYNA", tekrar_renk, [-1.8, -2], cam, 0.15)
        render_AN("CIKIS", çıkış_renk, [-0.9, -3.5], cam, 0.15)
        
        # Oyun açıklaması
        render_AN("OK TUSLARI ILE SECIM YAPIN", end_palette["obstacle"], [-3, -5], cam, 0.1)
        render_AN("ENTER VEYA SPACE ILE ONAYLAYIN", end_palette["obstacle"], [-3.3, -5.5], cam, 0.1)
...
def main():
    ...
    # Ana oyun döngüsü için tekrar oynama döngüsü
    retry = True
    ...
    # Ana kamera oluştur (başlangıç ve oyun sonu ekranları için)
    main_cam = camera("main_cam", vec3(0,0,0), [[1,0,0],[0,1,0],[0,0,1]], True)
    main_cam.move(vec3(0,-3,-13))
    
    # Başlangıç ekranını göster
    show_start_screen(mwin, main_cam)
    
    while retry and not glfw.window_should_close(mwin):
        dt = 0.001
        game_tick = 0
        a = 1 # global alpha
        ...
        # Oyun çökmesi durumu için değişken
        crash = False
        death_message = ""
        
        print("Starting...")
        while not glfw.window_should_close(mwin) and not crash:
            glfw.poll_events()
...
##                palette_transform = True
                ...
                if o.pos.z > 10:
                    ...
                    if o.check_collision():
                        if player.shields_remaining <= 0:
                            ...
                            death_message = random.choice(death_msgs)
                            print("\n", death_message)
                            # Burada hemen çıkmak yerine game over ekranına geçeceğiz
        ...
        # Oyun çöktüğünde game over ekranını göster
        if crash:
            retry = show_game_over_screen(mwin, main_cam, score, death_message)
        else:
            retry = False
